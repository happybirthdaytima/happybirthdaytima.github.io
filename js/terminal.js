class LinuxTerminal {
    ...
    init() {
        ...
        // Исправленный обработчик системной ошибки
    ...
    processCommand() {
        ...
        setTimeout(() => {
            ...
            // ВСЕГДА создаем новую строку после команды
            setTimeout(() => this.addNewCommandLine(), 100);
            
            // Системная ошибка только когда нет активного квеста
    ...
    showSystemError() {
        // Стилизованная ошибка в стиле консоли
        this.addOutput("", "error");
        this.addOutput("kernel: [ 1234.567890] BUG: unable to handle kernel NULL pointer dereference at 0000000000000000", "error");
        this.addOutput("kernel: [ 1234.567891] PGD 0 P4D 0", "error");
        this.addOutput("kernel: [ 1234.567892] Oops: 0000 [#1] SMP NOPTI", "error");
        this.addOutput("kernel: [ 1234.567893] CPU: 0 PID: 1337 Comm: birthday_quest Tainted: G        W", "error");
        this.addOutput("", "error");
        this.addOutput("⚠️  СИСТЕМНАЯ ОШИБКА: Нажмите любую клавишу для продолжения...", "warning");
    ...
    startRandomQuest() {
        ...
        // Сбрасываем текущий квест при запуске нового
        this.currentQuest = null;

        // Если все квесты использованы, сбрасываем
        if (this.usedQuestIndexes.size >= this.quests.length) {
            this.usedQuestIndexes.clear();
        }

        // Ищем неиспользованный квест
    ...
    echo(args) {
        ...
        if (answer === this.currentQuest.answer) {
            ...
            // НЕ сбрасываем квест - позволяем попробовать снова
        }
    }
    ...
    completeQuest() {
        ...
        this.currentQuest = null; // Сбрасываем текущий квест
...
Доступные команды:
  ...
  neofetch  - информация о системе
    ...
    addNewCommandLine() {
        ...
        // Удаляем старую командную строку если есть
...
Memory: 32GB Quest RAM
    ...
    showNetwork() { this.addOutput("eth0: flags=4163<UP,BROADCAST,RUNNING,MULTICAST>  mtu 1500", 'output'); }
    showDiskSpace() { this.addOutput("Файл.система   Размер Использовано  Доступно Использовано%", 'output'); }
    showKernelInfo() { this.addOutput("Linux birthday 6.2.0-26-generic", 'output'); }
}
